...
class App:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if x > self.cam.image.shape[1]:  # width
                if y > 60: # Text Bauteilzustand
                    ...
                    if y < 200:
                        self.io_state = True
                        self.logger.info(" IO Button wurde gedrückt - IO Status ändern")
                    # NIO Button
                    elif 200 <= y < 340:
                        self.io_state = False
                        self.logger.info(" NIO Button wurde gedrückt - IO Status auf False setzen")
                    # Save Button
                    else:
                        self.save_img = True
                        self.img_counter = self.num_images_to_save
                        self.logger.info(" Save Button wurde gedrückt - Prozess starten")
    ...
    def draw_ui(self):
        ...
        if self.save_img:
            ...
            #GPIO
            # Setze Pin 17 auf HIGH um Motor zu starten
            self.gpio_controller.write_pin(17, True)
            #time.sleep(0.05)

        # GPIO Ausgang Motor zurücksetzen
        if not self.save_img:
            self.gpio_controller.write_pin(17, False)
    ...
    def run(self):
        ...
        # GPIO
        # Konfiguriere Pin 17 als Output --> für Motor
        self.gpio_controller.setup_pin(17, 'output')
        # Konfiguriere Pin 27 als Output --> für Leuchte
        self.gpio_controller.setup_pin(27, 'output')
        # Setze Pin 27 auf HIGH
        self.gpio_controller.write_pin(27, True)
